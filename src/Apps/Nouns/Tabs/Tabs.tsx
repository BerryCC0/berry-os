/**
 * Nouns Tabs
 * Browse Nouns DAO contracts and organizations
 */
...
import ContractHelpers from './components/ContractHelpers';
...
export default function Tabs({ windowId }: TabsProps) {
  const [activeTab, setActiveTab] = useState<Network>('mainnet');
  const [expandedContract, setExpandedContract] = useState<string | null>(null);

  const mainnetContracts: Contract[] = [
    {
    ...
    {
      name: "DAO Admin [Configuration]",
      address: "0xd7c7c3c447Df757a77C81FcFf07f10bB22d98f4a"
    },
    {
  ...
  const sepoliaContracts: Contract[] = [
    {
      name: "DAO Governor",
  ...
  const metagovOrganizations: Contract[] = [
    {
      name: "Nouncil",
  ...
  const renderTabContent = () => {
    const contracts = activeTab === 'mainnet' ? mainnetContracts
      : activeTab === 'sepolia' ? sepoliaContracts
      : activeTab === 'l2' ? l2Contracts
      : activeTab === 'metagov' ? metagovOrganizations
      : nounsTokens;

    // Only show helpers for mainnet contracts
    const showHelpers = activeTab === 'mainnet';

    return (
      <div className={styles.contractsList}>
        {contracts.map((contract, index) => {
          const { mainName, description } = parseContractName(contract.name);
          const contractKey = `${activeTab}-${index}`;
          const isExpanded = expandedContract === contractKey;
          
          return (
            <div key={index} className={styles.contractItem}>
              <div 
                className={`${styles.contractHeader} ${showHelpers ? styles.clickable : ''}`}
                onClick={() => {
                  if (showHelpers) {
                    setExpandedContract(isExpanded ? null : contractKey);
                  }
                }}
              >
                <span className={styles.contractName}>
                  ...
                  <span className={styles.contractNameBold}>{mainName}</span>
                  {description}:
                </span>
                  ...
                  {contract.chains && contract.chains.length > 0 ? (
                    contract.chains.map((chain, chainIndex) => (
                      <span key={chainIndex}>
                        ...
                        {chainIndex < (contract.chains?.length ?? 0) - 1 && ', '}
                      </span>
                    ))
                  ) : contract.links && contract.links.length > 0 ? (
                    contract.links.map((link, linkIndex) => (
                      <span key={linkIndex}>
                        <a 
                          href={link.url}
                          target="_blank"
                        ...
                        {linkIndex < (contract.links?.length ?? 0) - 1 && ', '}
                      </span>
                    ))
                  ) : (
                    <a 
                      href={`https://${activeTab === 'sepolia' ? 'sepolia.' : ''}etherscan.io/address/${contract.address}`}
                      target="_blank"
                      rel="noopener noreferrer"
                      className={styles.link}
                    >
              ...
              {/* Show helpers for mainnet contracts when expanded */}
              {showHelpers && isExpanded && (
                <ContractHelpers 
